 ...
 */
function PhantomJsHeadlessAnalyzer(appJsFilePath, senchaDirOrAppJson, pageRoot, pageToProcess, includeAllScriptTags, failOnError, serverPort) {
...
PhantomJsHeadlessAnalyzer.prototype.normaliseFilePath = function (filePath) {
    if (/^http:/.test(filePath)) {
        filePath = filePath.replace(this.host, "");
    } else {
...
PhantomJsHeadlessAnalyzer.prototype.setHtmlPageToProcess = function (tempPage) {
    var htmlString,
        turnUrlIntoRelativeDirectory = buildUrlConverter(this.host);
...
function buildUrlConverter(host) {
    return function turnUrlIntoRelativeDirectory(relativeTo, url) {
        if (/^file:/.test(url)) {
            url = url.substring(5);
        }
        if (/^http:/.test(url)) {
...
PhantomJsHeadlessAnalyzer.prototype.startWebServerToHostPage = function (tempPage, callback) {
    var pathSepReplacement = new RegExp("\\" + path.sep, "g"),
        url = this.host + tempPage.replace(pathSepReplacement, "/");

    this.app = connect()
        .use(connect["static"](process.cwd()))
        .listen(this.port, function (err) {
            grunt.log.debug("Connected: " + url + "  -  " + process.cwd());

            callback(url);
        })
        .on('error', function (err) {
            grunt.fail.fatal('Could not start static file web server: '+ err);
        });
};
...
PhantomJsHeadlessAnalyzer.prototype.getDependencies = function (doneFn, task) {
    ...
    var spawnOptions = {
            ...
            options: {
                inject: [
                    require.resolve('../assets/injected-script.js')
                ],
                loadImages: false
            },
    ...
    this.startWebServerToHostPage(tempPage, function (url) {
        // Spawn phantomjs
        phantomjs.spawn(url, spawnOptions);
    });
};
